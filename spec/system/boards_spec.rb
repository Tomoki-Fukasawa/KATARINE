...
RSpec.describe 'ツイート編集', type: :system do
  ...
  context 'ツイート編集ができるとき' do
    it 'ログインしたユーザーは自分が投稿したツイートの編集ができる' do
      ...
      expect(
        all('.more')[1].hover
      ).to have_link'edit',href: edit_tweet_path(@tweet1)
      # 編集ページへ遷移する
      ...
      # 投稿内容を編集する
      fill_in 'tweet_image',with: "#{@tweet1.image}+gazourl"
      fill_in 'tweet_text',with: "#{@tweet1.text}+text"
  ...
  context 'ツイート編集ができないとき' do
    it 'ログインしたユーザーは自分以外が投稿したツイートの編集画面には遷移できない' do
      ...
      expect(
        all(.more)[0].hover
      ).to have_no_link 'edit',href: edit_tweet_@path(@tweet2)
    ...
    it 'ログインしていないとツイートの編集画面には遷移できない' do
      ...
      expect(
        all('.more')[1].hover
      ).to have_no_link 'edit', href: edit_tweet_path(@tweet1)
      # ツイート2に「編集」へのリンクがないことを確認する
      expect(
        all('.more')[2].hover
      ).to have_no_link 'edit', href: edit_tweet_path(@tweet2)
    end
  end
end
...
RSpec.describe 'ツイート削除', type: :system do
  ...
  context 'ツイート削除ができるとき' do
    it 'ログインしたユーザーは自らが投稿したツイートの削除ができる' do
      ...
      expect(
        all('.more')[1].hover
      ).to have_link 'destroy', href:tweet_path(@tweet1)
      # 投稿を削除するとレコードの数が1減ることを確認する
      expect{
        all('.more')[1].hover.find_link('destroy',href: tweet_path(@tweet1)).click
        sleep 1
      }.to change{Tweet.count}.by(-1)
  ...
  context 'ツイート削除ができないとき' do
    ...
    it 'ログインしていないとツイートの削除ボタンがない' do
      ...
      expect(
        all('.more')[1].hover
      ).to have_no_link 'destroy', href:tweet_path(@tweet1)
      # ツイート2に「削除」へのリンクがないことを確認する
      expect(
        all('.more')[0].hover
      ).to have_no_link 'destroy', href:tweet_path(@tweet2)
    end
  end
end
...
RSpec.describe 'ツイート詳細', type: :system do
  ...
  it 'ログインしたユーザーはツイート詳細ページに遷移してコメント投稿欄が表示される' do
    ...
    expect(
        all('.more')[1].hover
      ).to have_link'show',href: tweet_path(@tweet)
    # 詳細ページに遷移する
